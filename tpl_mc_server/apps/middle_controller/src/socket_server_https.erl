...
start(Port, DataPid, PNum) ->
	start(Port, DataPid, spawn(fun() -> undef_output() end), PNum, nil, nil)
.
...
loop(Socket, DataPid, MessagePid, Port) ->
	case ssl:recv(Socket, 0, ?TCP_RECV_TIMEOUT) of
		{ok, Packet} ->
...
get_body(Sock, Length, HeaderList) ->
	case ssl:recv(Sock, 0, ?TCP_RECV_TIMEOUT) of
		...
		{ok, http_eoh} -> 
			case Length of
				0 -> {"", Length, HeaderList};
				_ -> ssl:setopts(Sock, [{packet, raw}]),
					case ssl:recv(Sock, Length, ?TCP_RECV_TIMEOUT) of
						{ok,Body} ->
			...
			%% inet:setopts(Sock, [{packet, raw}]),{ok,Body}=ssl:recv(Sock, Length, 60000),	Body;
...
send_unsupported_error(Sock) ->
	ssl:send(Sock, "HTTP/1.1 405 Method Not Allowed\r\nConnection: close\r\nAllow: POST\r\nContent-Type: text/html; charset=UTF-8\r\nCache-Control: no-cache\r\n\r\n"),
	ssl:close(Sock)
.	
